      ...
      >
        ...
        <div className="hidden md:flex items-center gap-10">
          <ul className="flex gap-8 text-[15px] font-normal tracking-[0.1em] capitalize items-center">
            ...
            <li className="relative group py-2">
              <div
                ...
                className="cursor-pointer hover:text-orange-500 transition flex items-center gap-1"
              >
                About Us{" "}
                <span className="text-[10px]">
                  <FontAwesomeIcon
                    icon={isAboutOpen ? faChevronUp : faChevronDown}
                    className="text-sm"
                  />
                </span>
              </div>
              <ul className="absolute left-0 mt-2 w-48 bg-white text-gray-800 py-4 shadow-xl rounded-sm border-t-2 border-orange-500 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                {dropdownLinks.map((link, index) => (
                  <li
                    ...
                    className="px-6 py-2 hover:text-orange-500 cursor-pointer transition text-[14px]"
                  >
                    {link.name}
                  </li>
                ))}
          ...
          <div className="flex gap-5 text-sm ml-4 ">
            <a
              href="#"
              className={`transition-colors duration-300 ${scrolled ? "text-gray-600 hover:text-orange-500" : "text-white hover:text-orange-400"}`}
            >
                ...
                <ul className="mt-3 ml-4 flex flex-col gap-3 border-l-2 border-orange-500 pl-4">
                  {dropdownLinks.map((link, index) => (
                    <li
                      key={index}
                      onClick={() => handleScroll(link.id)}
                      className="text-gray-500 hover:text-orange-500 cursor-pointer text-sm normal-case"
                    >
                      {link.name}
                    </li>
                  ))}
